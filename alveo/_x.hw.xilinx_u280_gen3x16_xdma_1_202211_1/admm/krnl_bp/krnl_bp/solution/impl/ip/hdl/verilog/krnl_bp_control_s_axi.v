...
#(parameter
    ...
    output wire [63:0]                   mat_p,
    output wire [63:0]                   vec_q,
    output wire [63:0]                   out_r,
    output wire [31:0]                   rho,
    output wire [31:0]                   alpha,
    output wire [31:0]                   N,
    output wire [31:0]                   chunk_size,
...
localparam
    ...
    ADDR_IER               = 7'h08,
    ADDR_ISR               = 7'h0c,
    ADDR_MAT_P_DATA_0      = 7'h10,
    ADDR_MAT_P_DATA_1      = 7'h14,
    ADDR_MAT_P_CTRL        = 7'h18,
    ADDR_VEC_Q_DATA_0      = 7'h1c,
    ADDR_VEC_Q_DATA_1      = 7'h20,
    ADDR_VEC_Q_CTRL        = 7'h24,
    ADDR_OUT_R_DATA_0      = 7'h28,
    ADDR_OUT_R_DATA_1      = 7'h2c,
    ADDR_OUT_R_CTRL        = 7'h30,
    ADDR_RHO_DATA_0        = 7'h34,
    ADDR_RHO_CTRL          = 7'h38,
    ADDR_ALPHA_DATA_0      = 7'h3c,
    ADDR_ALPHA_CTRL        = 7'h40,
    ADDR_N_DATA_0          = 7'h44,
    ADDR_N_CTRL            = 7'h48,
    ADDR_CHUNK_SIZE_DATA_0 = 7'h4c,
    ADDR_CHUNK_SIZE_CTRL   = 7'h50,
    WRIDLE                 = 2'd0,
    WRDATA                 = 2'd1,
    WRRESP                 = 2'd2,
    WRRESET                = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_mat_p = 'b0;
    reg  [63:0]                   int_vec_q = 'b0;
    reg  [63:0]                   int_out_r = 'b0;
    reg  [31:0]                   int_rho = 'b0;
    reg  [31:0]                   int_alpha = 'b0;
    reg  [31:0]                   int_N = 'b0;
    reg  [31:0]                   int_chunk_size = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_MAT_P_DATA_0: begin
                    rdata <= int_mat_p[31:0];
                end
                ADDR_MAT_P_DATA_1: begin
                    rdata <= int_mat_p[63:32];
                end
                ADDR_VEC_Q_DATA_0: begin
                    rdata <= int_vec_q[31:0];
                end
                ADDR_VEC_Q_DATA_1: begin
                    rdata <= int_vec_q[63:32];
                end
                ...
                ADDR_CHUNK_SIZE_DATA_0: begin
                    rdata <= int_chunk_size[31:0];
                end
            endcase
        end
    end
...
assign mat_p         = int_mat_p;
assign vec_q         = int_vec_q;
assign out_r         = int_out_r;
assign rho           = int_rho;
assign alpha         = int_alpha;
assign N             = int_N;
assign chunk_size    = int_chunk_size;
// int_interrupt
always @(posedge ACLK) begin
    if (ARESET)
        int_interrupt <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_MAT_P_DATA_0)
            int_mat_p[31:0] <= (WDATA[31:0] & wmask) | (int_mat_p[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_MAT_P_DATA_1)
            int_mat_p[63:32] <= (WDATA[31:0] & wmask) | (int_mat_p[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_VEC_Q_DATA_0)
            int_vec_q[31:0] <= (WDATA[31:0] & wmask) | (int_vec_q[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_VEC_Q_DATA_1)
            int_vec_q[63:32] <= (WDATA[31:0] & wmask) | (int_vec_q[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_chunk_size[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CHUNK_SIZE_DATA_0)
            int_chunk_size[31:0] <= (WDATA[31:0] & wmask) | (int_chunk_size[31:0] & ~wmask);
    end
end
