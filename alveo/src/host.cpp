...
int main(int argc, char** argv) {
    ...
    OCL_CHECK(err, cl::Buffer buffer_in1(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
        vector_size_bytes * DATA_SIZE, matrix_p.data(), &err));
    ...
    std::vector<float> Ax = matrixVectorMultiply(matrix_A_std, std::vector<float>(source_hw_results.begin(), source_hw_results.end()), DATA_SIZE, DATA_SIZE);
